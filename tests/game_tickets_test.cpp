...
int main() {
    ...
    if(!tickets.issue("one",1,"auth",now))return 1;
    ...
    if(wins!=1)return 2;
    tickets.issue("expires",1,"auth",now);
    if(tickets.consume("expires",now+meta::GameTickets::lifetime))return 3;
    tickets.issue("old",1,"auth",now);tickets.issue("new",1,"auth",now);
    if(tickets.consume("old",now) || !tickets.consume("new",now))return 4;
    for(int i=0;i<4096;++i)if(!tickets.issue(std::to_string(i),i,"auth",now))return 5;
    if(tickets.issue("overflow",5000,"auth",now))return 6;
    if(!tickets.issue("collected",5000,"auth",now+meta::GameTickets::lifetime))return 7;
    return 0;
}
