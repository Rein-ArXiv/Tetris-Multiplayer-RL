...
Part 10 에서는 여기서 만든 봇/클라이언트 경로 위에 **`tetris_meta` 메타 서버와 랭킹** 을 얹는다. Part 9 가 "학습된 정책을 게임 프로세스 안으로 들여오는 법" 이었다면, 다음 장은 "그 클라이언트를 토큰·ELO·리더보드가 있는 서비스에 연결하는 법" 이다.

즉 다음 장의 관심사는 모델 정확도가 아니라 서비스 경계다. guest 토큰 발급, relay 의 `/v1/auth/verify` 검증, `MATCH_SUMMARY` 교차검증, `/v1/matches` 저장, 그리고 게임오버 화면의 ELO 델타 표시가 이어진다.

**적용 범위 주의** — Part 10 의 meta 통합은 **C++ 게임 클라이언트 (`tetris.exe`) 와 릴레이 (`tetris_relay`)** 에 적용된다. 이 장에서 만든 Python `netbot` 은 현재 **`--connect` direct HELLO/SEED 핸드셰이크 경로 전용** 이다 — `QUEUE_JOIN` 토큰 제출, `READY` 수락 로비, `MATCH_SUMMARY` 송신, `MATCH_RESULT` 처리가 모두 구현돼 있지 않으므로 랭킹 매칭에는 참여하지 않는다. 프레임 enum (`python/netbot/framing.py`) 에는 확장 메시지 타입이 정의되어 있어 향후 구현 시 참고 가능. 학습/회귀 테스트에 봇을 쓸 땐 C++ 호스트 (`./tetris --host`) 한 쪽 + Python netbot 클라이언트 조합으로 충분하다.
