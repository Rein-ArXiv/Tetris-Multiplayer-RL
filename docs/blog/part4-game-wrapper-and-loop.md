...
- **선행 상태:** Part 1의 규칙 엔진과 Part 2~3의 플랫폼/표현 계층.
- **이번 장의 파일:** `src/game.{h,cpp}`, `src/main.cpp`, `core/constants.h`.
- **연결점:** `Game`은 `SimGame`을 위임·표현하고, `main.cpp`는 입력·모드와
  60Hz fixed-step 수명을 소유한다.
- **완료 게이트:** Single 모드에서 렌더 FPS와 무관하게 같은 입력/시드가 같은
  `StateHash()`를 만들고 메뉴→게임→게임오버→재시작이 동작해야 한다.

## 들어가며

Part 1에서 게임 로직을, Part 2~3에서 창과 렌더러를 만들었다. 이제 이것들을
`Game`과 `main.cpp`의 **게임 루프**로 묶는다.
...
이 장의 실제 파일 경계는 `src/game.{h,cpp}`, `src/main.cpp`,
`core/constants.h`, `core/input.h`다.

---

## 1. `Game`은 SimGame과 화면 사이의 어댑터다

Part 1의 `SimGame`은 화면과 장치를 모른다. Part 3의 renderer는 테트리스 규칙을
모른다. `Game`이 둘 사이에서 상태를 읽어 픽셀로 바꾸고, `main.cpp`가 객체의
수명과 호출 타이밍을 결정한다.

```mermaid
graph TB
    M[main.cpp<br/>AppMode·입력·수명·60Hz] --> G[Game<br/>위임·렌더·표현 이벤트]
    G --> S[SimGame<br/>결정론적 상태 전이]
    G --> R[renderer<br/>draw_rect/text/image]
    G -. Part 5 .-> A[audio]
    M --> P[platform<br/>시간·키·마우스·창]
```

상속 대신 composition을 쓴다. `Game`이 `SimGame sim`을 멤버로 가지므로 Python
학습은 `SimGame`만 사용할 수 있고, C++ 클라이언트만 `Game`의 표현 기능을 쓴다.

Part 4 체크포인트의 핵심 공개 계약은 다음과 같다.

```cpp
// Part 4 체크포인트: src/game.h 핵심
class Game
{
public:
    explicit Game(uint64_t seed = 0);

    void SubmitInput(uint8_t inputMask) { sim.SubmitInput(inputMask); }
    void Tick()                         { sim.Tick(); }
    void Draw();
...
`gameOver`와 `score`는 별도 상태가 아니라 `sim.gameOver`, `sim.score`의 참조
별칭이다. 값을 복사하면 wrapper와 코어가 서로 다른 점수·종료 상태를 갖게 되므로
반드시 생성자 initializer에서 연결한다.
...
`Draw()`는 읽기만 한다. `Grid()`, `GhostBlock()`, `CurrentBlock()`, `NextBlock()`을
renderer 호출로 바꿀 뿐 `SimGame` 상태를 변경하지 않는다. Part 5에서 audio
handle과 이벤트 소비가 `Game`에 추가되지만 `SubmitInput`/`Tick` 공개 계약은
유지된다.
...
sequenceDiagram
    ...
    M->>P: begin_frame + input
    loop accumulator >= 1/60s
        M->>G: SubmitInput(mask)
        G->>S: SubmitInput(mask)
        M->>G: Tick()
        G->>S: Tick()
    end
    M->>R: renderer_begin
    M->>G: Draw()
    G->>R: draw_rect / draw_text
    M->>R: renderer_end
```
...
`platform_key_pressed()`는 "이번 프레임에 처음 눌린" 키만 감지하는 엣지 트리거다 (Part 2에서 구현: `state[key] && !prev[key]`). 이 값은 **한 프레임만** true이다.
...
### 4.3 해결: AccumulateInput / ConsumeInput
...
### 5.2 해결: deltaTime 클램핑

```cpp
// platform/win32.cpp:365 (SDL 백엔드는 platform/sdl.cpp:321 에 동일 코드)
...
`SimGame::SubmitInput`은 각 비트를 순서대로 처리한다. 비트 처리 순서만 보이도록 **예시(단순화)** — 실제 저장소에는 `gameOver` 가드와 소프트 드롭 레이트 제한이 더 붙는다:

```cpp
// 예시(단순화) — 비트 처리 순서만 강조. 실제 코드는 아래 전체 인용 참조.
...
실제 `src/sim_game.cpp` 의 `SubmitInput` 은 두 가지가 더 붙는다 — (1) 맨 앞의 `if (gameOver) return;` 가드, (2) `INPUT_DOWN`(소프트 드롭) 을 매 틱 그대로 적용하지 않고 `softDropCounterTicks` 로 3틱(≈15셀/초) 마다 한 번만 반응하게 게이트하는 로직. 이 카운터는 상태 해시에 포함되므로 양쪽 클라이언트에서 동일하게 전개돼 결정론을 깨지 않는다:
...
멀티플레이 모드에서 루프 구조가 변경된다. 아래는 **흐름을 보여주는 단순화 프리뷰** — 실제 `src/main.cpp` 의 구현(시작 카운트다운 가드, `GetRemoteInput` 의 out-param + 미도착 시 `break`, gameOver 체크 등)은 Part 6 §5.3 에서 1:1 로 인용한다:

```cpp
// 단순화 프리뷰 (실제 코드는 Part 6 §5.3 참조)
while (accumulator >= SECONDS_PER_TICK)
{
    uint8_t inputMask = ConsumeInput();
    ...
    if (app == AppMode::Net && session.isConnected())
    {
        ...
        //    (실제 코드: ri 는 GetRemoteInput(simTick, ri) out-param 으로 받고,
        //     미도착이면 break — 아래는 흐름만 강조한 형태)
...
양쪽 피어의 입력이 모두 도착한 틱까지만 시뮬레이션을 진행한다. 한쪽 피어의 입력이 늦으면 다른 쪽의 시뮬레이션도 대기한다. 이것이 **Lockstep** 동기화의 핵심이다. Part 6에서 상세히 다룬다.
...
다음 Part 5에서는 `Game`이 시뮬레이션 이벤트를 오디오로 소비하는 경계를
완성한다. Part 6에서는 이 고정 틱 위에 TCP lockstep을 구축한다.
...
2. **테스트 가능성.** Sim은 headless(창·렌더러 없음)로 돌아가야 한다.
   `sim_hash_dump`와 Python binding 결정론 테스트가 그 경계를 검증한다.
   `test_placement_parity.py`는 입력 전개 진리표를 검증하며 C++ Sim 전체를 직접
   비교하는 테스트는 아니다. Sim에 renderer 심볼이 섞이면 headless 링크가 깨진다.
...
`SimGame` 은 매 틱 내부에서 라인 클리어 / T-스핀 / 가비지 수신 / 게임 오버가 발생하면 자기 멤버 변수에 "이번 틱에 이런 일이 있었다" 라는 플래그를 적는다. 틱 루프가 한 번 읽고 나서 초기값으로 리셋한다.

| 필드 | 의미 | 트리거 |
|------|------|--------|
| `lastLinesCleared` | 이번 틱에 지운 줄 수 (0~4) | `Tick()` 내 라인 클리어 로직 |
| `lastTSpinLines` | 이번 틱 T-스핀으로 지운 줄 수 (없으면 `-1`) | T-스핀 판정 + 라인 클리어 |
| `lastGarbageReceived` | 이번 틱에 바닥에서 올라온 가비지 줄 수 | 가비지 큐 소비 |
| `gameOverEvent` | 이번 틱에 톱아웃 발생 (bool) | 스폰 실패 검출 |

이 네 필드는 Sim 이 매 틱 **덮어쓰지 않는다**. 해당 이벤트가 실제로 발생한 틱에만 설정되고, 틱 루프 쪽에서 소비 후 초기값으로 클리어한다. 즉 "edge event" 플래그다. `lastTSpinLines` 의 "없음" 표식이 0 이 아니라 `-1` 인 이유는 0 줄 T-스핀(스핀만 하고 줄은 안 지움)도 유효한 이벤트이기 때문이다.
...
graph TB
    subgraph Sim["SimGame (결정론)"]
        ...
        T --> F2["lastTSpinLines"]
        T --> F3["lastGarbageReceived"]
        T --> F4["gameOverEvent"]
    end
    subgraph Loop["틱 루프 (main.cpp)"]
        AS["apply_self_fx()<br/>(shakeLeft)"]
        AP["apply_peer_fx()<br/>(shakeRight)"]
    end
    subgraph FX["비결정론 보조 레이어"]
        SH["shake_trigger<br/>(가비지/게임오버만)"]
        CO["trigger_callout<br/>/ trigger_tspin_callout"]
        AU["오디오 트리거<br/>(Part 5)"]
    end
...
Sim 의 네 플래그가 한 방향으로만 흐른다는 것이 핵심이다. 보조 레이어는 Sim 을 읽기만 할 뿐 되돌려 쓰지 않는다. 이 단방향성이 결정론 경계를 지킨다. 또한 shake 는 콜아웃과 달리 **라인 클리어가 아니라 가비지 수신 / 게임 오버에만** 걸린다는 점을 기억하라 (§A.3 의 람다에서 다시 확인한다).
...
두 람다의 구조를 뜯어보자. 두 람다는 사실상 **같은 본문**이고, 자기/상대 보드에 각각 `shakeLeft` / `shakeRight` 두 개의 독립적인 shake 상태를 주입한다는 점만 다르다. (왼쪽이 내 보드, 오른쪽이 상대 보드.)

**공통 로직:**

- `lastTSpinLines >= 0` 이면 T-스핀 콜아웃을 우선 띄운다. T-스핀이 아니고 `lastLinesCleared > 0` 이면 일반 라인 클리어 콜아웃. 즉 콜아웃은 **라인 클리어 / T-스핀** 에 반응한다.
- shake 는 **라인 클리어에는 걸지 않는다**. `lastGarbageReceived > 0` (가비지 수신) 이면 중간 강도 shake (6px, 0.20s), `gameOverEvent` 면 가장 강한 shake (16px, 0.50s) 만 건다. 라인 클리어로 화면을 흔들지 않는 이유는, 공격(가비지) 이 반대편 보드로 넘어가면 그쪽 람다가 그쪽 shake 를 걸어 주기 때문이다 — 일반적인 테트리스 대전 연출 관례다. `shake_trigger` 는 "현재 활성 강도보다 약하면 무시" 정책이라, 게임오버 shake 는 직전의 가비지 shake 를 덮어쓴다.
- 마지막 네 줄에서 **플래그 네 개를 전부 리셋한다** (`lastLinesCleared`, `lastTSpinLines`→`-1`, `lastGarbageReceived`, `gameOverEvent`). if 블록에 안 걸린 플래그도 초기값으로 밀어버리는 게 중요하다 — 다음 틱에서 "지난 번 이벤트의 찌꺼기" 를 다시 읽지 않도록. `lastTSpinLines` 의 초기값은 0 이 아니라 `-1` 임에 주의(§A.2).

**`apply_self_fx` vs `apply_peer_fx`:** 본문이 동일하므로 자기 보드든 상대 보드든 같은 연출 규칙을 탄다. 차이는 주입되는 shake 상태(`shakeLeft` vs `shakeRight`) 뿐이다. 콜아웃 텍스트는 각각 자기 보드 위(`coLocal`)·상대 보드 위(`coRemote`) 에 뜬다.
...
safeTick 까지 캐치업 루프 안에서 매 틱 호출. 즉 한 프레임에 여러 틱이 진행되면, shake 트리거도 여러 번 걸릴 수 있다. `shake_trigger` 의 "약한 건 무시" 정책이 여기서 제 역할을 한다 — 캐치업 5틱 동안 가비지 shake 가 연달아 와도, 먼저 활성화된 가장 강한 shake 가 살아남는다.
...
Part 5 에서 XAudio2 를 붙일 때, "블록 락 / 라인 클리어 / 가비지 수신" 사운드 트리거가 자연스럽게 `apply_self_fx` 에 들어간다. Sim 은 그대로이고 람다만 확장한다. 가상적으로:
...
따라서 두 피어가 각자 다른 FPS 로 렌더링하면 shake RNG 소비 속도도 다르지만, Sim 결과는 완벽히 일치한다. Part 6 에서 이야기하는 HASH 검증이 shake 를 무시해도 안전한 것은 이 분리 덕분이다.

### B.5 Part 3 투영 행렬과의 접점

`renderer_set_view_offset(dx, dy)` 가 실제로 하는 일은 Part 3 에서 다룬 orthographic 투영 행렬의 translation 컴포넌트를 매 프레임 갱신하는 것이다. 2D 렌더러의 view 변환은 다음 형태를 갖는다:
...
자세한 행렬 구조와 셰이더 쪽 uniform 업로드는 [Part 3: 2D 렌더링](./part3-rendering-and-ui.md) 의 orthographic 투영 섹션을 참고.
...
Sim 은 자기가 흔들리고 있는지 모른다. 렌더러는 자기가 왜 흔들리는지 모른다. 틱 루프만 양쪽을 알고, 그 둘 사이의 유일한 통로는 `ShakeState` 객체 하나다. 이 격리가 Part 6 Lockstep 과 Part 9 RL 봇의 헤드리스 실행을 모두 지탱한다.

---

## 부록 C. Part 6에서 적용할 네트워크 나가기 모달

이 부록의 Single/BotSingle 경로는 지금 적용할 수 있지만, Net 분기는 Part 6의
`Session`과 `AppMode::Net`이 완성된 뒤 추가한다. 아래 네트워크 코드를 Part 4
체크포인트에 미리 넣지 않는다.
...
**`inGame` 판정.** 세 모드 각각에서 게임 객체(ownership unique_ptr) 가 실제로 세팅되어 있는지까지 확인한다. 예를 들어 `AppMode::Net` 이지만 아직 `MATCH_FOUND` 를 받기 전이라 `gameLocal == nullptr` 인 상태에서는 X 버튼을 띄우지 않는다. 대기 화면에서 X 버튼은 Part 6 의 별도 취소 경로가 담당한다 (이 모달과 혼동하면 안 됨).
...
`session.Close()` 가 TCP 연결에 shutdown 신호를 보내고 세션 소유 handle을 정리한다. 상대 피어의 `Session::ioThread` 는 `recv()` 에서 EOF 를 받고 `isConnected` 를 false 로 떨어뜨린다. 상대 쪽 메인 루프에서는 "연결 단절" 을 감지하는 기존 코드가 (Part 6 의 Section B 에서 구현) 이를 "상대방이 게임을 나갔다 = 내 승리" 로 해석한다. 즉 나는 `GAME_OVER_CHOICE` 같은 별도 메시지를 명시적으로 보내지 않는다 — 단순히 연결을 내리는 것 자체가 "나 나갑니다" 의 신호로 해석되는 프로토콜이다.

이 동작이 Part 6 의 DESYNC 처리 경로와 구분되는 점: DESYNC 는 양쪽이 계속 연결된 채로 `HASH` 가 불일치해서 "누가 맞는가" 의 투표 경로로 들어간다. 여기 나가기는 투표 이전에 한쪽이 링크 자체를 내려버리는 것이다. 상대는 "승리" 표시로 넘어가고 내 쪽은 바로 메뉴로 나간다.
...
실제 재현 명령은 Part 6 의 매뉴얼 테스트 섹션 참조. 게임 루프 쪽의 관심사는 `tickPauseForDialog` 가드 한 줄의 효과가 모달 동안 내 보드에서 눈으로 보이는 변화로 연결된다는 것뿐.
...
- Disabled 항목이 생기면 클릭해도 반응하지 않아야 하고 키보드 Enter 도 무시해야 한다. 현재 "Single vs Bot" 은 내장 휴리스틱 봇 때문에 모델 파일이 없어도 열리지만, 코드에는 로스터가 비는 비정상 상황을 막는 방어 분기가 남아 있다.
...
```cpp
// Part 4 체크포인트: src/main.cpp
// ── 메뉴 ────────────────────────────────────────────────────────────
//   키보드와 마우스 모두 지원. 키보드 위/아래 로 menuIndex 이동, Enter 선택.
//   마우스 클릭 = 해당 항목 즉시 활성화 (menuIndex 와 무관).
if (app == AppMode::Menu)
{
    int titleW = measure_text("TETRIS", 64);
    draw_text("TETRIS", (720 - titleW) / 2, 80, 64, WHITE);
...
이 블록은 `activated` 패턴에 집중하기 위해 후속 메뉴를 생략한 예시다. 최종
소스는 Part 10의 `Customize`, Part 11의 `Settings`까지 포함해 7개 메뉴다.

코드 구조가 세 단계다.

1. **키보드 커서 이동** — `PKEY_DOWN`/`PKEY_UP` 엣지로 `menuIndex` 를 수정. 마우스와 독립. 이걸 엣지 트리거로 처리하는 이유는 Part 4 본문 4장에서 설명한 입력 누적과 동일 — held 상태로 하면 프레임당 여러 칸씩 점프한다.
...
3. **키보드 활성화** — Enter/Space 엣지가 들어오면 `activated = menuIndex`. disabled 보호(`!(menuIndex == 1 && !botAvailable)`) 는 방어 코드다 — 현재 기본 로스터에는 `Heuristic (test)` 가 항상 들어가므로 정상 빌드에서 이 분기는 거의 타지 않는다.
...
`activated == 1` 이 곧장 게임을 시작하지 않고 `AppMode::BotSelect` 로 넘어가는 점이 현재 구조의 핵심이다. 처음에는 단일 `model/policy.onnx` 를 바로 로드하는 설계로 충분했지만, 학습 알고리즘이 늘어나면서 C++ 클라이언트가 `model/*.onnx` 와 `model/bots/*.onnx` 를 스캔해 로스터를 만들도록 확장했다. 실제 ONNX 로드는 사용자가 BotSelect 화면에서 특정 항목을 고른 뒤에만 수행한다. 내장 휴리스틱 항목은 로드가 필요 없다.

### D.4 disabled 항목 처리

현재 "Single vs Bot" 은 `model/policy.onnx` 유무로 비활성화되지 않는다. 로스터 생성 함수가 항상 `Heuristic (test)` 를 먼저 넣기 때문이다. 그래도 메뉴 루프에는 "로스터가 비어 있으면 선택을 막는다" 는 방어 코드가 남아 있다. 이 경우 해당 버튼은 **gui 함수를 호출하지 않는다**. 대신 `draw_rect_rounded` 로 배경을 직접 그리고, 라벨 색을 `DISABLED` (어두운 회색) 로 찍는다.
...
키보드 쪽에서는 Enter 분기에서 `!(menuIndex == 1 && !botAvailable)` 로 한 번 더 막는다. 이중 가드다 — 사용자가 Up/Down 으로 disabled 항목에 커서를 올릴 수는 있어도 Enter 를 쳐도 아무 일도 안 일어난다. 현재 `botAvailable` 은 정상 로스터에서 항상 true지만, 방어 분기가 있어도 일반 메뉴 입력 흐름은 복잡해지지 않는다.

버튼 바로 아래에 작은 설명("`(no bot roster entries found)`") 을 붙여 왜 회색인지 사용자가 추측하지 않아도 된다.
