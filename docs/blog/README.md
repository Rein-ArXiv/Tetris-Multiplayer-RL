...
[검증 기록](../polish-validation.md)에 있다. Part 0~18은 아래 의존 관계를 따르는 누적 구현 교재다. 접속 보호는 [Part 16: WSS·입장권](part16-secure-admission.md)이 설명한다.
...
graph TB
    ...
    P15 --> P16
    P16 --> P17[Part 17<br/>계정 해시·폐기·복구]
    P17 --> P18[Part 18<br/>서버 경기 판정·실패 상태]
...
- **온라인 서비스·출시:** 클라이언트 기반 → Part 5 → 6 → 7 → 10 → 11 → 12. 오디오, lockstep, relay, 계정·결과 영속화, 사용자 설정, 운영 검증 순서다. Part 12 의 용량 측정이 한 대의 상한을 보여 준 뒤, 그 상한을 올려야 할 때 [Part 14](./part14-event-loop-scaling.md) 로 간다. 공개 접속 보호와 계정 보존은 Part 16 → 17 → 18까지 이어 읽는다.
...
| 17 | [Part 17: 가입 없는 계정 보존](part17-guest-account-recovery.md) | 해시 이관·키 교체·복구 파일·응답 유실 재시도·네이티브 계정 UI |
| 18 | [Part 18: 서버 경기 판정](./part18-authoritative-results.md) | PvP 입력 검증·결과 사유·엄격한 JSON·화면과 계정 책임 분리 |
...
보안·단절·재시도 같은 횡단 관심사는 **그 상태를 소유한 글에 구현 원리**를 둔다. relay의 연결 제한과 종료 수명은 Part 7, DB 멱등성과 인증 경계는 Part 10, 서버 승패 판정은 Part 18이 설명한다. Part 12는 같은 내용을 다시 구현하지 않고 실제 배치, 백업, 장애 전환, 부하 측정과 릴리스 게이트로 묶는다. 독자는 기능을 만들 때 실패 계약을 함께 배우고, 운영 단계에서는 전체 시스템 관점으로 다시 검증할 수 있다.
...
- meta와 proxy가 같은 호스트면 `127.0.0.1`에 bind한다. 별도 배치에서 meta를 저전력
