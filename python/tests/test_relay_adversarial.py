...
from netbot.framing import (FramingError, MsgType, RejectReason, build_frame,
                            parse_frames)
...
def test_wrong_room_codes_run_out_of_budget_before_the_code_space_does(
    ...
    거절됨" 을 구분할 수 없고, 구분하지 못하면 예산을 통째로 들어내도 이 테스트가
    통과한다.
    """
    _, port = unranked
    src = "127.0.7.7"          # 다른 테스트와 버킷을 섞지 않는다

    rejected_at = None
    for attempt in range(1, 40):
        s = socks(_connect(port, src))
        s.sendall(_room_join("ZZZZZ"))
        buf = bytearray()
        try:
            payload = _recv_frame(s, MsgType.SERVER_REJECT, buf, 2.0)
        except (AssertionError, TimeoutError, OSError, RuntimeError):
            payload = None
        s.close()
        if payload is not None and payload[0] == int(RejectReason.ROOM_GUESS_LIMIT):
            rejected_at = attempt
            break

    assert rejected_at is not None, (
        "40번을 연속으로 틀렸는데도 예산이 소진되지 않았다 — 오답에 값이 매겨지지 "
        "않고 있으므로 코드 공간을 시간만 들이면 훑을 수 있다")
    assert rejected_at > 1, (
        "첫 시도부터 거절됐다 — 코드를 잘못 받아 적은 정상 사용자가 즉시 막힌다")

    # 다른 주소는 영향을 받지 않아야 한다. 상한이 전역이면 한 명이 모두를 막는다.
    other = socks(_connect(port, "127.0.7.8"))
    other.sendall(_room_create())
    obuf = bytearray()
    payload = _recv_frame(other, MsgType.ROOM_INFO, obuf, 5.0)
    code_len = payload[0]
    code = payload[1:1 + code_len]

    joiner = socks(_connect(port, "127.0.7.9"))
    joiner.sendall(_room_join(code))
    jbuf = bytearray()
    info = _recv_frame(joiner, MsgType.ROOM_INFO, jbuf, 5.0)
    assert info[1 + info[0]] != 2, "정상 코드로 들어가는 길까지 막혔다"
...
def test_matching_then_going_silent_costs_the_silent_side_not_the_victim(
    ...
    """
    LOBBY_SEC = 2
    _, port = relays(["--lobby-timeout-sec", str(LOBBY_SEC)])

    silent_ip = "127.0.8.1"     # 매칭된 뒤 아무것도 안 보내는 쪽
    victim_ip = "127.0.8.2"     # READY 를 보내고 기다리는 쪽

    def one_abandoned_match():
        v = socks(_connect(port, victim_ip))
        s = socks(_connect(port, silent_ip))
        vbuf, sbuf = bytearray(), bytearray()
        v.sendall(_queue_join())
        s.sendall(_queue_join())
        _recv_frame(v, MsgType.MATCH_FOUND, vbuf, 5.0)
        _recv_frame(s, MsgType.MATCH_FOUND, sbuf, 5.0)
        v.sendall(_ready(1))          # 피해자는 제 몫을 다한다
    ...
    for attempt in range(1, 9):
        one_abandoned_match()
        probe = socks(_connect(port, silent_ip))
        probe.sendall(_queue_join())
        pbuf = bytearray()
        try:
            payload = _recv_frame(probe, MsgType.SERVER_REJECT, pbuf, 1.5)
        except (AssertionError, TimeoutError, OSError, RuntimeError):
            payload = None
        probe.close()
        if payload is not None and payload[0] == int(RejectReason.QUEUE_NO_SHOW):
    ...
    ok = socks(_connect(port, victim_ip))
    ok.sendall(_queue_join())
    obuf = bytearray()
    try:
        payload = _recv_frame(ok, MsgType.SERVER_REJECT, obuf, 1.5)
    except (AssertionError, TimeoutError, OSError, RuntimeError):
        payload = None
    assert payload is None, (
        f"READY 를 매번 보낸 피해자 주소가 거절됐다 (reason={payload[0]}). "
