...
좌우 홀드 반복은 DAS/ARR 방식이다. 처음 누른 순간은 `platform_key_pressed()` 가 누적해 즉시 1칸 이동하고, 이후 `HorizontalRepeatInput()` 이 약 10틱 대기한 뒤 2틱마다 한 번씩 같은 방향 비트를 추가한다. 양쪽 방향키가 동시에 눌리면 좌우 비트를 지워 중립으로 처리한다.
...
부록 C 의 `quitDialogOpen` 은 Part 5 에서 다룰 세션 단절 처리와 직결된다. "나가기 = 세션 Close" 가 네트워크 레이어에서는 "상대 소켓에 FIN" 으로 보이고, 그쪽 `Session::ioThread` 의 `recv()` 가 EOF 를 받는 순간 `isConnected = false` 로 떨어진다. Part 5 Section B 의 연결 단절 grace 복귀 / 승리 처리 로직이 이 EOF 를 소비한다.

부록 D 의 `activated` 패턴은 Part 5 에서 Room Lobby 의 Create/Join 버튼, Part 7 에서 Audio 설정 화면의 볼륨 슬라이더에서 같은 형태로 재사용된다. 한 UI 상태를 "이번 프레임에 발생한 이벤트" 변수 하나로 수렴시키는 접근이 키보드/마우스/터치 등 입력 소스가 늘어날 때마다 재확장 가능한 구조가 된다.
