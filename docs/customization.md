...
2026-09-19 기준. 엔진을 다시 만들 필요는 없다. **화면에 보이는 것**은 표현 계층에서,
...
사용자 설정은 `platform::settings_file_path()`가 정한 사용자 폴더의 `Tetris/settings.cfg`에 `idle_animation=0/1`로 저장된다. 실행 폴더의 테마 설정과 구분한다.
...
요소는 `src/main.cpp`·`src/gui.cpp`, 계정 화면은 `src/account_screen.cpp`를 조정해야 한다. 배포 가능한 라이선스의
...
| 낙하 속도·잠금·가비지 공격 | `src/sim_game.cpp`, `core/constants.h` | 양쪽 클라이언트·봇·서버 검증기에 동일 규칙 필요 |
...
6. PvP의 `server/ranked_game.h`와 봇 BP의 `bot/reward_replay.h`에서도 같은 규칙으로 판정되는지 확인한다.
7. 서로 다른 규칙의 클라이언트가 매칭되지 않도록 **ruleset 버전 협상/거절**을 추가한다.
...
아이콘을 바꾸려고 여기서부터 모두 읽을 필요는 없다. 현재 계정 UI는 `src/account_screen.*`, 계정 서비스는 `meta/account_client.*`로 분리돼 있다. 콘텐츠를 바꾸는 작은 확장 경계는:
