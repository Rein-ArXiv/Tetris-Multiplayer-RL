...
- `python/netbot/` 의 framing/session 포트와 parity 테스트로 C++ 세션과 Python 클라이언트가 같은 프로토콜을 공유하게 만들었다.
...
uv run pytest python/tests/test_framing_parity.py python/tests/test_checkpoint_roundtrip.py -q
```

기대 결과: Python 에서 `import sim` 이 성공하고, framing/checkpoint parity 테스트가 통과한다. C++ 쪽과 Python 쪽의 프레임 직렬화 결과가 동일해야 한다.
