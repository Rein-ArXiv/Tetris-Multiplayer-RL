...
(`begin_auth` 의 오프로드 제출 부분. 앞쪽의 unranked 통과·빈 토큰 거절 분기는 생략)
...
이 구조체는 단계마다 다른 필드를 쓴다. `join_code` 는 룸 진입 때만, `pause_credit` 은 백프레셔가 걸린 뒤에만 의미가 있다. 그래서 "단계별로 파생 클래스를 만들고 가상 함수로 디스패치하는" 설계가 자연스러워 보인다. 실제로 큰 프록시 구현에서는 그렇게 하기도 한다. 여기서 그렇게 하지 않은 이유는 세 가지다.
...
`verified`는 채널 소유 loop에서만 갱신한다. meta로 넘길 때에는 판정된 값만 복사하고, HTTP worker가 이 게임 상태를 수정하지 않는다. 결과 사유인 `result_status`도 같은 소유 경계에 있다. 규칙 검증은 Part 18을 따른다.
...
(`RelayLoop::run` 의 I/O 디스패치 부분. 앞뒤의 인계 수령·오프로드 continuation 실행·만기 처리는 생략)
...
그리고 **멈춤을 도입하면 멈춤을 전제로 하던 다른 계산이 전부 틀어진다.** 이 함수의 절반이 재개 처리인 이유가 그것이다. 입을 막아 둔 연결은 읽기 이벤트를 못 받으니 유휴 시계가 얼어붙고, 그대로 두면 풀리자마자 "15초간 조용했다" 는 판정으로 끊긴다 — 우리가 막아 놓고 상대를 벌주는 셈이다. 바이트 레이트도 같다. 멈춰 있는 동안 상대의 커널 버퍼에 고인 것이 재개하는 순간 한 버스트로 들어오는데, 그 버스트는 상대가 규정을 넘겨 보낸 것이 아니라 **우리가 안 읽어서 생긴 적체**다. 일반화하면 이렇다: **흐름을 인위적으로 멈추는 기제를 넣을 때는, 시간을 재는 모든 값이 그 멈춤을 알고 있는지 확인해야 한다.** 모르는 값이 하나라도 남으면 그 값은 피해자를 가해자로 오인한다.

이 문장에는 값비싼 후일담이 붙는다. 처음 고른 답은 "재개 후 3초간 레이트를 면제한다" 였다. 적체를 청구하지 않는다는 방향은 맞았지만 면제를 **재개 시각에 묶은 것**이 틀렸다 — 재개는 보류 송신이 고수위 아래로 떨어질 때마다 일어나므로, 상대가 조금씩 빼내며 그 언저리를 오르내리게 하면 이전 창이 만료되기 전에 새 창이 걸린다. 면제가 영구히 갱신되고 상한은 한 번도 적용되지 않는다. 실측으로 3.5초에 352 MB, 초당 96 MiB 가 64 KiB/s 상한을 그대로 통과했다.

지금 형태가 그 구멍을 닫는 방식은 **면제를 시간이 아니라 양으로 주는 것**이다. 멈춰 있는 동안 버킷 시계를 세우고, 그 구간의 몫은 재개할 때 "pause 길이 × 상한" 만큼 일회성 예산으로 확정해 지급한다. pause 구간들은 서로 겹치지 않고 그동안 버킷은 멈춰 있으므로 두 몫이 같은 시간을 두 번 세지 않는다 — 어떤 구간에서든 통과량이 `버스트 한도 + 상한 × 경과 시간` 을 넘지 못한다는 성질이 그래서 성립한다. 옛 면제에는 이 성질이 없었다.

여기서 두 번째 교훈이 나온다. **면제를 설계할 때 물어야 할 것은 "얼마나 오래 봐줄 것인가" 가 아니라 "이 면제가 갱신될 수 있는가" 다.** 시간 창은 언제나 갱신될 수 있고, 갱신될 수 있는 면제는 면제가 아니라 우회로다. 크기를 그 사건 자신의 크기에서 유도하면 — 여기서는 pause 의 길이에서 — 갱신시킬 것이 남지 않는다.
...
지금은 연결당 하드 상한과 프로세스 전역 예산으로 메모리를 묶고, 정상 유휴·상호 pause·상대의 송신 배수 정지를 각각 시간으로 처리한다. 우리가 읽기를 멈춘 연결에 일반 유휴 벌점을 주지 않되, 그 pause를 풀어 줄 상대의 진행도 확인한다. 위 `Stage::Forward` 분기가 무한 정지를 막는 자리다. 원칙은 이렇게 바뀐다: **멈춤의 끝은 멈춤을 일으킨 자원의 단위로 재라.** 메모리 때문에 멈췄으면 끝도 메모리로 재고, 시간으로 재는 만기는 멈춤과 무관한 다른 실패(사라진 피어)를 위해 남겨 둔다. 두 축을 섞으면 한 축의 상한이 다른 축의 원인을 처벌한다.
...
이 시점에는 `Conn` 이 이미 사라졌을 수 있어 보류 버퍼를 붙일 자리가 없다. 채널이 붙들고 있는 소켓 복사본으로 한 번 시도하고, 커널이 받아 주지 않으면 포기한다. 프레임이 작고(정수 세 개와 결과 상태 1바이트) 상대가 그 순간 읽고 있을 확률이 높으므로 실용적인 타협이다. 다만 이건 **의식적으로 남겨 둔 최선 노력(best-effort) 경로**이며, 결과가 반드시 전달돼야 한다면 답은 더 큰 버퍼가 아니라 클라이언트가 재접속 후 결과를 조회하는 별도 경로다. 보장이 필요한 전달을 연결 수명에 묶지 않는 것 — 이것도 이 프로젝트를 떠나서도 반복되는 규칙이다.
...
- **한 매치의 두 소켓과 채널** — 복제할 필요가 없다. 이 상태는 그 매치 바깥의 누구도 보지 않는다. A 의 바이트는 B 에게만 가고, 입력 검증과 서버 승패 판정도 그 채널 안에서 끝난다. 매치 하나가 통째로 어느 루프에 있든 다른 매치는 알 필요가 없다.
...
`begin_forwarding` 의 샤딩 분기만 인용한다. 분기를 타지 않았을 때 이어지는 단일 루프 경로(두 연결을 포워딩 단계로 옮기고 idle 타이머를 거는 부분)는 생략했다.
