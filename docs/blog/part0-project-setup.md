...
이 시리즈는 그 반대로 간다. raylib 을 링크하지 않는다. Windows 에서는 Win32 API 로 창을 만들고 wgl 로 OpenGL 컨텍스트를 요청한다. macOS/Linux 에서는 SDL2 만 빌려와서 창·입력만 맡기고 나머지는 직접 쓴다. 오디오는 XAudio2 또는 SDL audio subsystem, 폰트는 GDI `wglUseFontBitmaps`(Windows) 또는 단일 헤더 `stb_truetype` TTF 글리프 아틀라스(SDL2), 이미지 디코딩은 GDI+(Windows) 또는 단일 헤더 `stb_image`(비-Win32), MP3 디코딩은 단일 헤더 `dr_mp3`, 네트워킹은 WinSock2/BSD 소켓 순정, 봇 추론은 ONNX Runtime CPU 바이너리만 링크, Python RL 레이어는 pybind11 로 `SimGame` 을 노출한다. 이 모든 것이 하나의 `CMakeLists.txt` 에서 관리된다.
...
│   ├── text_stb.cpp       ← stb_truetype TTF 글리프 아틀라스 (SDL2 경로)
│   ├── shake.h/.cpp       ← 화면 흔들림
│   └── image.h/.cpp       ← PNG 아이콘/텍스처 쿼드
│
├── audio/                 ← 오디오 백엔드
│   ├── audio.h            ← 공용 인터페이스
