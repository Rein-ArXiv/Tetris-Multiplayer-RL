...
7. **다음 파트 연결고리** — "이 장에서 만든 X 가 Part N 에서 Y 의 기반이 된다" 한 문단.
...
>     uint32_t chk = fnv1a32(out.data(), out.size());
>     le_write_u32(out, chk);
>     return out;
> }
> ```
> 길이 2바이트 · 타입 1바이트 · payload · 체크섬 4바이트 순. 체크섬이 헤더 포함 전체를 덮는다는 점이 핵심 — 덕분에 중간 비트 뒤집힘과 길이 오염 둘 다 잡힌다."
...
각 파트 B.0 표 참조. **억지로 채우지 마라** — 설명이 자연스럽게 짧아지면 줄인다. 긴 발췌를 "채우기" 위해 중복 설명하지 않는다.
...
Part N+1 에서는 ... 를 다룬다. 이 장에서 만든 ... 가 ... 의 기반이 된다.
```
