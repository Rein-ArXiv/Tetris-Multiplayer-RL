...
> [Part 0: 셋업](./part0-project-setup.md) | [Part 1: Win32+GL](./part1-window-and-opengl.md) | **Part 2** | [Part 3: 테트리스 로직](./part3-tetris-logic.md) | [Part 4: 게임 루프](./part4-game-loop.md) | [Part 5: 네트워킹](./part5-lockstep-networking.md) | [Part 6: Python RL](./part6-python-rl.md) | [Part 7: 오디오](./part7-xaudio2-audio.md) | [Part 8: 릴레이 서버](./part8-relay-server.md) | [Part 9: RL + ONNX 봇](./part9-rl-onnx-bot.md)
...
이 시리즈의 전체 소스 코드는 실제 프로젝트의 `renderer/renderer.cpp` (276줄), `renderer/shaders.h` (98줄), `renderer/renderer.h` (52줄)에 해당한다. 텍스트는 플랫폼별로 `renderer/text_win32.cpp` (109줄, GDI) 또는 `renderer/text_stb.cpp` (229줄, 5x7 비트맵)로 분리되며, Section I에서 추가된 화면 흔들림은 `renderer/shake.cpp` (58줄), 이미지 렌더링은 `renderer/image.cpp` (372줄)이다.

> **참고:** Part 1의 노트와 마찬가지로, 본 시리즈는 raylib의 `DrawRectangle`을
...
저장소 `renderer/renderer.cpp` 라인 224-276 을 **한 글자도 빼지 않고** 인용한다. 리뷰어가 "분해 전략만 설명하고 실제 람다는 어떻게 생겼냐" 라고 지적했던 그 함수다. 코드 블록 아래에 람다 3 개 (`V`, `Rect`, `Corner`) 의 역할을 다시 풀어 쓴다.

```cpp
// renderer/renderer.cpp:224-276
void draw_rect_rounded(int x, int y, int w, int h, float roundness, Color c)
{
    float minDim = (float)(w < h ? w : h);
    float r = roundness * minDim * 0.5f;
...
Section I 에서 "테트리스!" / "백투백!" / "퍼펙트 클리어!" 같은 콜아웃 스프라이트와 게임 오버 UI 썸네일을 화면에 띄우기 위해 PNG 이미지 로더가 필요해졌다. `draw_rect` 와 `draw_text` 만으로는 꾸밀 수 없는 픽셀 아트 자산을 입히는 경로다. 구현은 `renderer/image.cpp` 한 파일에 모여있고, 디코더 (GDI+) 와 텍스처 렌더러 (OpenGL) 두 레이어로 나뉜다.
...
Windows 에서는 stb_image 를 링크하지 않는다. 대신 OS 가 이미 가지고 있는 GDI+ 의 `Gdiplus::Bitmap` 으로 PNG/JPG/BMP 를 읽는다. 이유는 단순하다 — 바이너리 크기를 늘리지 않고, GDI+ 는 이 프로젝트가 이미 폰트용으로 링크 중인 `gdi32` 와 형제 라이브러리라 추가 의존성이 0 이다.

`decode_image_win32` 의 핵심 루프:

```cpp
// renderer/image.cpp:112-169
...
교훈은 Part 2 섹션 2.4 와 동일하다 — **셰이더 컴파일/링크 결과는 반드시 로그로 뱉어라**. `image.cpp` 의 `compile_shader_s` / `link_program_s` 는 이제 `renderer.cpp` 의 대응 함수와 똑같이 `[SPRITE GLSL]` 태그를 붙여 stderr 로 출력한다.

### 8.6 빌드 체크포인트

이 시점에서 빌드하면 `image_init()` 호출만 해둬도 `image_load("assets/callout_tetris.png")` → `draw_image(h, 100, 100, 256, 128)` 로 투명 배경 PNG 스프라이트가 화면에 뜬다. 콜아웃 애니메이션 (등장/유지/페이드아웃) 은 `src/main.cpp` 의 타이머 쪽에서 `draw_image_tinted(h, x, y, w, h, {255,255,255, alpha})` 로 알파 값을 바꿔가며 호출한다.

---

## 9. 화면 흔들림 (`renderer/shake.cpp`)

Section I 는 "테트리스 클리어 / 가비지 피격 / 게임오버" 순간에 화면을 한 번 흔들어 타격감을 준다. 구현은 놀라울 정도로 작다 — 58줄짜리 `renderer/shake.cpp` 하나다. 핵심 설계는 **렌더 단계에서만 뷰 오프셋을 주입** 한다는 점이다. Sim 은 건드리지 않으므로 네트워크 결정론이 깨지지 않는다.
...
이 숫자들은 순전히 플레이 느낌 기준이다 — 너무 작으면 안 느껴지고 너무 크면 UI 를 읽기 어렵다. 실측 기준은 "피격 당한 순간 1~2 프레임 안에 '맞았다' 는 걸 인지하되, 셀 경계가 보일 정도로 큰 흔들림은 아니어야 한다".
...
`renderer/text_win32.cpp` 는 Part 2 섹션 7 에서 다룬 GDI + `wglUseFontBitmaps` 경로다. 이것은 **Windows 전용** 이다. Part 0 에서 CMake 에 `if(WIN32)` 분기를 심어두었기 때문에, macOS / Linux 빌드 (SDL2 기반) 에서는 다른 파일이 컴파일된다 — `renderer/text_stb.cpp`.
...
3. **TTF 로더 추가 시 바이너리 크기 증가** — stb_truetype 를 넣을까 고민했지만 (헤더 단일 파일이라 기술적 장벽은 없음), 현재 UI 에 필요한 건 ASCII 만이고 외관상 픽셀 폰트가 오히려 어울린다.

결론은 **저장소의 자체 5x7 비트맵 폰트** 로 간다. 파일 상단 주석이 이 선택을 명시한다:
...
파일명에 `stb` 가 붙은 것은 이름값이라기보다 "향후 stb_truetype 로 교체할 슬롯" 이라는 자리표시. 지금은 TTF 대신 **내장 5x7 비트맵** 을 쓴다.

### 10.2 내장 비트맵 폰트의 구조

`kFont5x7` 배열은 95개 글리프 (ASCII 32~126) × 5 바이트 = 475 바이트짜리 테이블이다. 각 글리프는 5개의 열(column) 로, 각 바이트의 비트 0~6 이 위→아래 7 픽셀을 나타낸다:

```cpp
// renderer/text_stb.cpp:56~
...
열3: 0x11 = 0001_0001   →  X...X
열4: 0x7E = 0111_1110   →  .XXXXXX.
```

이 데이터는 널리 쓰이는 퍼블릭 도메인 "standard 5x7" 폰트에서 추출한 것이다.

### 10.3 `draw_text` — Batched Quads 전략

렌더링은 단순하다:

1. 문자열의 각 문자 → 95개 중 하나의 글리프 인덱스 (ASCII 32 = index 0).
2. 각 글리프의 "on" 픽셀마다 작은 정사각형 quad (`size/7` px 한 변) 를 꼭짓점 버퍼에 추가.
3. 문자열 끝까지 다 누적하면 `glBufferData` + `glDrawArrays(GL_TRIANGLES, ...)` **한 번** 으로 전체를 그린다.

한 줄 텍스트의 꼭짓점 수 — 극단적 가정 (한 문자당 `5*7 = 35` 픽셀 모두 on, 20자) 으로도 $20 \times 35 \times 6 = 4200$ 꼭짓점. 실제로는 글리프 당 평균 15 픽셀 on 이라 $\approx 1800$ 꼭짓점. 여전히 16 KB 수준이라 단일 `glBufferData` 에 여유있게 들어간다.

**rect 셰이더 재사용** 이 핵심이다. 별도 폰트 전용 셰이더를 만들지 않는다 — 어차피 단색 quad 의 집합이므로 `draw_rect` 가 쓰는 `kRectVert/kRectFrag` 와 동일한 VAO/VBO 를 빌려쓴다. `text_stb.cpp` 상단의 `extern` 선언이 이 공유를 가능하게 한다:
...
구현 내용은 다르지만 호출 측 (src/main.cpp 의 UI 코드) 에서는 구분할 수 없다. `measure_text` 가 반환하는 픽셀 폭은 `(strlen(text) * 6 * (size/7))` 패턴으로 계산되어 UI 레이아웃이 양쪽 백엔드에서 시각적으로 거의 동일하다.

`renderer_load_font` 는 text_stb 에서 **no-op** 이다 — TTF 를 안 쓰니까 파라미터를 받아놓고 무시. 추후 stb_truetype 로 전환할 때 이 훅이 필요해진다.
...
Windows 빌드 → `text_win32.cpp` + monogram.ttf 로 부드러운 안티앨리어싱 텍스트.
macOS/Linux 빌드 → `text_stb.cpp` + 내장 5x7 로 레트로 픽셀 텍스트.

두 쪽 모두 "TETRIS" 메뉴 타이틀이 동일 위치에 뜬다. 대소문자, 숫자, 기본 기호 (ASCII 32~126) 는 다 지원. 한글은 미지원 (5x7 테이블에 없음) — 필요해지면 stb_truetype 도입으로 넘어갈 예정.
...
8. **`platform_end_frame()` 이 `SwapBuffers` 를 호출하는가** — 극단적으로 드물지만, 프레젠테이션 경로가 고장나면 백 버퍼에 그려도 화면에 안 나타난다. Part 1 에서 다룬 `SwapBuffers(hdc)` / `SDL_GL_SwapWindow` 연결을 다시 점검.

순서의 의도는 "가능성 높은 원인부터 체크 비용 낮은 순". 1~3 번은 1분 안에 확인 가능하고, 실제로 버그의 80% 가 거기서 잡힌다.
...
`image_shutdown` 과 `renderer_text_shutdown` 도 `renderer_shutdown` 과 동일한 시점 (platform 종료 전) 에 호출되어야 한다. 호출 순서는 `image_shutdown` → `renderer_text_shutdown` → `renderer_shutdown` 이 권장되며, 이는 "상위 모듈 → 하위 모듈" 의존 순서를 따른다.
...
1. **매 프레임 입력 상태 조회 API** — Part 1 에서 `platform_mouse_x`, `platform_mouse_down`, `platform_mouse_pressed` 를 이미 만들어 뒀다. IMGUI 는 이 API 들 위에 올라간다.
2. **즉시 모드 드로잉** — `draw_rect`, `draw_rect_rounded`, `draw_text`. 모두 "지금 이 픽셀 찍어" 계열. 1·2 파트에서 완성.
3. **폰트 측정** — 텍스트를 버튼 중앙에 정렬하려면 `measure_text(label, fontSize)` 가 있어야 한다. Part 2 초반에 만든 `s_font_cache` 의 `GetTextExtentPoint32` 경로가 이걸 제공한다.

이 세 조각이 모두 있으니 GUI 레이어는 전체 80줄짜리 `.cpp` 하나로 끝난다.
...
Part 1 에서 `WM_MOUSEMOVE` 로 갱신해 두던 `g_mouseX/g_mouseY` 를 여기서 처음 소비한다. 반개구간 `[x, x+w)` — 박스가 정확히 w 픽셀을 차지하게 해서 인접 박스 경계에서의 중복 hover 를 막는다.
...
### 12.4 `gui_close_button` — X 아이콘 직접 그리기
...
### 12.5 키보드 병행 — 마우스 전담 GUI 와 충돌 없이
...
`gui_modal_dim` 은 alpha=180 의 검은 사각형으로 전 화면을 덮는다. 블렌딩은 Part 2 초반에 `glEnable(GL_BLEND) + glBlendFunc(SRC_ALPHA, ONE_MINUS_SRC_ALPHA)` 로 이미 설정 — 그대로 반투명이 쌓인다. 모달 창(예: 게임 오버 화면) 을 이 위에 겹쳐 그리면 배경 게임이 어두워져 주의가 앞으로 옮겨간다.
...
## 13. 멀티플레이 Next 블록 프리뷰 (`DrawNextMini`)

지금까지 Part 2 는 단일 플레이어 보드를 전제로 했다. 멀티/봇 모드에서는 좌·우 두 보드를 나란히 그린다 — 그리고 각자의 "다음 블록" 미리보기가 필요하다.

### 13.1 문제 상황

`AppMode::Single` 경로는 `Game::Draw()` 가 보드 + Next 블록 + 현재 블록 + 고스트 + HUD 를 한 번에 그린다. 화면 우측 공간이 넉넉하니 큰 Next 미리보기를 띄운다.

`AppMode::Net` / `AppMode::BotSingle` 경로는 보드 두 개를 11px + 300px + 60px gap + 300px = 671px... 원래 창 500px 에 맞추려면 보드 축소가 필요. 실제 프로젝트는 창을 730px 로 확장해 두 보드를 담았고, 두 보드 사이 **60px gap** 이 생겼다. 이 공간에 작은 Next 프리뷰 두 개(플레이어·상대)를 끼워 넣는다.

기존 `Game::Draw()` 를 그대로 부르면 큰 Next 미리보기 + 현재 블록 + HUD 가 모두 그려져 옆 보드를 침범한다. 멀티 경로는 **보드만 그리는** `DrawBoardAt(x, y)` 를 별도로 쓴다 (이 함수는 스크롤·오프셋 인자를 받는 Part 3 소개 예정). Next 프리뷰만 별도로 호출할 수 있게 새 메서드 하나를 추가한다.

### 13.2 `Game::DrawNextMini`
...
1. **`sim.NextBlock()`** — 시뮬레이션이 보관한 "다음 블록" 인스턴스(Part 3 에서 상세). `SimBlock` 에는 종류 id, 현재 회전, row/column offset 이 들어있다.
...
갭 중앙에 프리뷰를 넣을 때, 여유는 60px 지만 라벨 "Next"·"Opp" 까지 들어가야 한다. 폰트 16 기준 "Next" 의 너비가 약 48px. `midX = 316` (보드 오른쪽 끝 + 5) 으로 시작 위치 정하면, 4칸짜리 블록(최대 I-블록 가로 4) 이 `4 × cellSize = 60` 이 되면 딱 gap 에 맞는다.

실제 값: `cellSize = 14` → `4 × 14 = 56px`. 60px gap 에 4px 여유. 넘치지 않고 양쪽 보드를 침범하지 않음.

세로 배치:

- y=20: "Next" 라벨 (로컬)
- y=44: 로컬 프리뷰 시작. 블록이 최대 세로 4칸(I) → `4 × 14 = 56px`, y=100 에서 끝.
- y=160: "Opp" 라벨 (원격)
- y=184: 원격 프리뷰 시작. y=240 에서 끝.

로컬 프리뷰와 "Opp" 라벨 사이 60px 공간은 여유. 추후 상대 점수·공격 큐 인디케이터를 여기 끼울 수 있다(Section I 에서 실제로 콜아웃 텍스트가 이 영역을 스쳐간다).
...
### 13.5 왜 현재 블록과 고스트는 없는가

프리뷰는 딱 "다음 블록" 한 개만 그린다. 이유:
...
3. **상대 보드의 다음 블록은 "정보 노출"이 핵심** — 상대가 다음에 뭘 받을지 알면 공격 전략을 짤 수 있다. 이게 멀티 모드의 전술 레이어. 단일 플레이어 Next 와 같은 역할이되, 적용 대상이 상대편.

결정론과도 맞물린다. Part 5 의 락스텝 프로토콜은 시드를 공유하므로 상대의 백(bag)·다음 블록 순서를 양쪽이 모두 알 수 있다. UI 가 이를 숨길 이유는 없다 — 오히려 드러내서 전략의 깊이를 만든다.

### 13.6 이 시점에서 빌드하면

Net / BotSingle 모드로 진입했을 때 좌·우 보드 사이 공간에 작은 Next 프리뷰 두 개가 각 플레이어의 다음 블록을 실시간으로 보여준다. 블록이 한 번 놓일 때마다 `sim.NextBlock()` 이 다음 한 칸 앞으로 전진하고, 다음 프레임의 `DrawNextMini` 가 새 블록을 집어 그린다 — IMGUI 스타일로 상태 저장 없이 매 프레임 시뮬레이션에서 뽑아 쓴다.
