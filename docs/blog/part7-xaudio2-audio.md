...
> **Part 7** | [Part 0: 셋업](./part0-project-setup.md) | [Part 1: Win32+GL](./part1-window-and-opengl.md) | [Part 2: 2D 렌더링](./part2-2d-rendering.md) | [Part 3: 테트리스 로직](./part3-tetris-logic.md) | [Part 4: 게임 루프](./part4-game-loop.md) | [Part 5: 네트워킹](./part5-lockstep-networking.md) | [Part 6: Python RL](./part6-python-rl.md) | **Part 7: 오디오** | [Part 8: 릴레이 서버](./part8-relay-server.md) | [Part 9: RL + ONNX 봇](./part9-rl-onnx-bot.md)
...
이 글에서는 Windows의 네이티브 오디오 API인 **XAudio2**를 직접 사용하여 같은 기능을 구현한다. Part 1에서 `InitWindow()`를 Win32 API로 풀어냈듯, 여기서는 `InitAudioDevice()` + `LoadSound()` + `PlaySound()`를 XAudio2 + dr_mp3로 풀어낸다. 뒤이어 Linux/macOS 이식을 위한 **SDL2 오디오 백엔드**(`audio/sdl_audio.cpp`)를 같은 `audio.h` 인터페이스에 맞춰 구현한다. 두 백엔드는 빌드 타임에 선택되며, 위쪽 게임 코드는 한 줄도 바꾸지 않는다.

이 시리즈의 전체 소스 코드는 실제 프로젝트의 `audio/audio.h` (37줄), `audio/audio.cpp` (~370줄), `audio/sdl_audio.cpp` (~240줄)에 해당한다.
...
동시에 재생될 수 있는 효과음 최댓값을 실측으로 잡아보자. 최악의 경우를 나열한다:
...
XAudio2 는 Windows 전용이다. 헤더 `xaudio2.h` 와 링크 대상 `xaudio2.lib` / `ole32.lib` 모두 Windows SDK 에 들어있고, macOS/Linux 에는 존재 자체가 없다. 프로젝트를 Linux (Wayland 세션에서의 OpenGL) 나 macOS (Part 10 에서 다룰 예정) 로 옮기려면 두 가지 길이 있다:
...
- `want.samples = 1024`. 콜백이 한 번에 요청하는 프레임 수. 1024 프레임 ≈ **23.2 ms** 블록 (44.1 kHz). 이것이 SDL 백엔드의 라운드트립 지연 하한이다. XAudio2 의 내부 블록은 보통 10 ms 이므로 SDL 쪽이 대략 **2 배 더 지연**된다. 테트리스에서는 인지되지 않는 수준이다.
...
| 라운드트립 지연 | \~10 ms | \~23 ms (`samples=1024 @ 44.1 kHz`) |
...
### 11.1 CMakeLists.txt 변경 (XAudio2 경로)
...
### 11.2 CMakeLists.txt 변경 (SDL 경로)

SDL 빌드는 다음과 같이 둘 중 하나의 `.cpp` 만 포함시키는 조건부로 구성한다. 동시에 두 파일을 빌드하면 같은 심볼 (`audio_init` 등) 이 중복 정의되어 링크 실패한다.

```cmake
if(WIN32)
    list(APPEND TETRIS_GAME_SOURCES audio/audio.cpp)
    target_link_libraries(tetris PRIVATE xaudio2 ole32)
else()
    list(APPEND TETRIS_GAME_SOURCES audio/sdl_audio.cpp)
    find_package(SDL2 REQUIRED)
    target_link_libraries(tetris PRIVATE SDL2::SDL2)
endif()
```

Linux 에서는 `apt install libsdl2-dev` (Debian/Ubuntu) 또는 `pacman -S sdl2` (Arch), macOS 에서는 `brew install sdl2` 로 SDL2 를 얻는다.
...
| 지연 | 라운드트립 | \~10 ms | \~23 ms (samples=1024) |
| 플랫폼 | | Windows | Windows / Linux / macOS |

이 시리즈를 통해 OS 창 생성(Part 1), GPU 렌더링(Part 2), 게임 로직(Part 3), 게임 루프(Part 4), 네트워킹(Part 5), RL 학습(Part 6), 그리고 오디오(Part 7)까지 -- 게임 엔진의 주요 서브시스템을 모두 직접 구현했다. 게다가 오디오는 **같은 API 밑에 두 개의 백엔드**를 얹어, 플랫폼 이식이 실제로 어떻게 작동하는지까지 보여줬다. raylib, SDL, Unity 같은 프레임워크의 각 모듈이 내부에서 무엇을 하는지 이해하면, 어떤 게임이든 스스로 만들 수 있는 기반이 된다.
...
Part 8 에서는 지금까지 만든 클라이언트를 연결하는 **릴레이 서버**를 다룬다. 이 장에서 만든 오디오 시스템은 Part 8 의 변화와 직접적인 상호작용은 없지만, 두 `Game` 인스턴스 (로컬 + 원격) 가 오디오를 공유하는 멀티플레이 경로가 그 위에서 더 공격적으로 시험된다. Part 9 에서는 이 오디오 이벤트들 (특히 `clearSoundEvent`) 이 RL 봇의 보상 신호 설계에 힌트로 쓰인다.
