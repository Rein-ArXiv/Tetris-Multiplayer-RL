...
int main() {
    ...
    if(db.purchaseIcon(p->token,"ruby",buyer)!=meta::IconPurchaseResult::Ok || !buyer || buyer->bp!=p->bp)return 7;
    // Spending BP does not replenish the daily earning allowance.
    if(db.saveBotWin(p->id,"after-purchase","lumen")!=0 || db.getByToken(p->token)->bp!=p->bp)return 8;
    return 0;
}
