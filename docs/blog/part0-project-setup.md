...
앞의 세 절이 "빌드하는 데 필요한 것" 이라면, 이것은 **빌드된 게임이 실제로 뜨는 데 필요한 것**이다. 렌더러가 OpenGL 3.3 Core 프로파일을 요구하므로 그래픽 드라이버가 그 프로파일을 줘야 한다. 2010년 이후에 나온 GPU 는 대부분 만족하고, 통합 그래픽도 마찬가지다. 조건이 맞지 않으면 [Part 3](./part3-rendering-and-ui.md) 의 렌더러가 시작하면서 어떤 GL 진입점을 못 받았는지 이름을 나열하고 종료한다.
...
- 이후 장에서 쓸 SDL2 와 OpenGL 개발 패키지의 설치 방법을 알고, 이 기계의 드라이버가 OpenGL 3.3 Core 를 주는지 확인했다.
