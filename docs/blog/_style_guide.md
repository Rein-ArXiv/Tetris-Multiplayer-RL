...
> [시리즈 목차](./README.md) · [이전: Part N-1](...) · **Part N** · [다음: Part N+1](...)

---
```

전체 13개 링크를 매 장에 반복하지 않고 이전/현재/다음만 둔다.
...
- `현재 소스 발췌`는 **코드 라인이 최종 저장소와 1:1 일치**해야 한다. 붙이기 전에
  반드시 해당 파일을 Read 해서 대조하라. 라인 범위는 대략치가 아니라 **실제 범위**
  여야 한다. 일부만 인용할 때도 `경로:시작-끝` 을 그 일부의 실제 범위로 적고,
  **함수 중간을 잘라내지 마라** (함수는 통째로 인용한다).
...
- `Part N 체크포인트`는 그 장까지 소개한 기능만 가진 컴파일 가능한 중간 상태다.
  최종 소스와 다르면 블록 **바로 아래**에 한 줄로 확장 지점을 밝힌다:
  > 이 시점의 `X` 는 A/B 만 처리한다. C 는 [Part 9](./part9-rl-onnx-bot.md) 에서 추가한다.
