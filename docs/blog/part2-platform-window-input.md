# Part 2: 플랫폼 계층 — 창, OpenGL 컨텍스트와 입력

> **시리즈:** 제로부터 멀티플레이어 테트리스 + RL까지
> [시리즈 목차](./README.md) · [이전: Part 1 — SimGame](./part1-deterministic-simulation.md) · **Part 2** · [다음: Part 3 — 렌더링과 UI](./part3-rendering-and-ui.md)

---

## 이 장의 구현 계약

- **선행 상태:** Part 1의 `SimGame`은 완성됐지만 아직 화면에 연결하지 않는다.
- **이번 장의 파일:** `platform/platform.h`, `platform/win32.cpp`,
  `platform/sdl.cpp`, `renderer/gl_defs.h`.
- **연결점:** 위 계층에는 창·시간·키·마우스·OpenGL 컨텍스트의 공통 API만 노출한다.
- **완료 게이트:** 선택한 백엔드에서 창 생성, 이벤트 펌프, 입력 edge/level,
  frame time과 buffer swap을 확인한다.
...
이 장의 실제 파일 경계는 `platform/win32.cpp`와 `platform/platform.h`다. 동일 인터페이스의 SDL2 백엔드는 `platform/sdl.cpp`로, macOS/Linux에서 기본 사용된다.
...
타이틀바 높이: ~31px, 테두리: ~1px 양쪽   (테마/DPI 의존, 측정 아님)
AdjustWindowRect 결과: rect = {-1, -31, 501, 621}   (예시 값)
전체 창 크기: 502 x 652
```

위 보정값(`-1, -31, ...`)과 "~31px" 는 특정 테마·DPI 에서 나오는 **예시**다. 실제 값은 Windows 버전·테마·DPI 스케일에 따라 달라지므로 측정한 상수가 아니라 감을 잡기 위한 수치로 본다.
...
`WS_THICKFRAME`(리사이즈 가능 테두리)를 넣지 않은 이유: 게임의 렌더링 해상도가 고정(현재 코드 기준 720x640)이므로, 사용자가 임의로 크기를 변경하면 비율이 깨진다. 고정 크기 창이 가장 안전하다.
...
// 초기화 시
QueryPerformanceFrequency(&s_freq);  // QPC 틱 주파수 (환경마다 다름)
...
QPC 주파수는 하드웨어·OS 에 따라 다르므로 실제 값은 `QueryPerformanceFrequency` 로 읽어야 한다. 흔히 10,000,000(10MHz, 즉 100나노초 정밀도)인 경우가 많지만 모든 하드웨어가 그렇지는 않다 (측정 아님, 환경 의존). 어느 쪽이든 `GetTickCount` 의 ~15ms 보다 훨씬 정밀하다.
...
이 글에서 구현한 `platform/win32.cpp`가 raylib의 `InitWindow()` 한 줄이 숨기는 일을 직접 수행한다. 핵심 개념을 요약하면:
...
다음 Part 3에서는 이 창 위에 OpenGL로 사각형과 텍스트를 그리는 **2D 렌더링 파이프라인**을 구축한다.
...
실제 `CMakeLists.txt` 는 `target_sources` 를 쓰지 않는다. 공통 소스를 `TETRIS_GAME_COMMON` 변수에 모아 두고, `TETRIS_USE_SDL2` 값에 따라 백엔드 소스 3개(`platform/*`, `renderer/text_*`, `audio/*`)를 더해 **두 분기에서 각각 `add_executable(tetris ...)`** 를 정의한다:
...
전체 구성은 Part 0 §3.4 에서 자세히 다룬다. 핵심은 `TETRIS_USE_SDL2` 한 값이 세 파일(창·텍스트·오디오)을 동시에 갈아끼운다는 점이다. Windows 에서도 `-DTETRIS_USE_SDL2=ON` 을 주면 SDL 백엔드로 빌드할 수 있다. 동일 게임 바이너리를 두 경로로 동작시켜 비교하며 디버깅하기 좋다.
...
기대 결과: Win32 빌드와 **시각적으로 동일한** 720x640 창이 뜨고, 테트로미노가 떨어지고, Space 로 하드드롭, Up 으로 회전이 된다. 텍스트 폰트만 미세하게 다르다(Win32 는 GDI `wglUseFontBitmaps`, SDL 은 `stb_truetype` 아틀라스).

이 시점에서 확인할 가치가 있는 건 "게임 로직이 플랫폼에 의존하지 않는다"는 실제 증거다. `game.cpp`, `renderer.cpp`, `main.cpp` 는 단 한 줄도 수정하지 않았는데 세 OS 에서 같은 게임이 돈다. 이게 `platform.h` 인터페이스를 얇게 유지한 대가다.
...
**2. 절전 상태 복귀 후의 점프.** 노트북 뚜껑을 닫아 S3 슬립에 진입했다가 깨우면 QPC 가 몇 시간 점프할 수 있다. deltaTime 이 10000.0f 로 튀면 시뮬레이션이 폭발한다. 이 방어는 Part 2 본문에서 이미 다룬 **0.1초 클램핑**이 그대로 처리한다:
...
반환 타입이 **double** 인 이유가 있다. float 로는 16.7 백만 초(약 194일) 이후에 1 밀리초 이하 정밀도를 잃는다. double 은 사실상 프로그램이 동작하는 모든 기간 내내 나노초 수준 정밀도를 유지한다. 테트리스 한 판이 몇 분짜리라 큰 차이는 없지만, **네트워크 시드 생성** (Part 6) 에서 `(uint64_t)(platform_get_time() * 1000000.0)` 로 마이크로초 타임스탬프를 만들 때 double 정밀도가 유용하다.
...
그래서 본문 작성 이후 `platform.h` 에 **마우스 6종 API 를 덧붙였다**. 게임 플레이 경로는 건드리지 않고(`game.cpp` · `main.cpp` 의 틱 루프 한 줄도 수정하지 않음), 메뉴 · 상점 · 프로필 같은 2D UI 계층만 마우스를 쓴다. 이 부록은 그 6개 함수가 어떻게 WM_* 메시지와 SDL_Event 에서 올라오는지를 설명한다.
...
구체적인 UI 코드는 Part 3 의 2D 렌더링 파트에서 `gui_button()` 헬퍼를 지으며 다룬다. 여기서는 API 가 어떻게 조합되는지만 보여주면 충분하다.

```cpp
// 예시(저장소에 없음, Part 3 에서 본격 구현)
...
- Win32 메시지 루프, OpenGL 컨텍스트 생성, 프레임 시작/종료, 키보드/마우스 상태 저장소를 `platform/win32.cpp` 한 파일 안에 닫아 넣었다.
- `platform.h` 공용 인터페이스를 기준으로 Win32 와 SDL2 경로가 같은 상위 API 를 노출하도록 정리했다.
- DPI, 마우스 capture, 휠 누적 초기화, 고해상도 타이머 같은 실제 함정을 코드와 함께 정리했다.
...
기대 결과: 창이 정상 생성되고, 드래그/리사이즈/마우스 클릭/휠 입력이 끊기지 않는다. 창을 닫으면 크래시 없이 프로세스가 종료된다.
