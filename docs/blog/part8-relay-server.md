...
> **참고**: `MATCHMAKING.md` 는 "ELO + DB + HTTP API 까지 붙인 완전체" 의 설계도다. 이 장에서는 그중 **릴레이 + 매치메이킹 + 룸** 만 구현한다. DB/ELO/HTTP 는 추후 확장이며, 없어도 게임은 동작한다.
...
엔트리 파일은 100줄 남짓. 세 가지만 한다: 포트 열고, matcher 스레드 띄우고, accept 루프 돌기. 전체 코드:
...
이걸 분기하는 게 `playerConnThread` 의 단일 책임. 전체 코드는 다음과 같다.
...
매치메이커의 책임은 단순하다: **대기 중인 연결을 FIFO 로 두 개씩 묶는다**. ELO 고려, 지역 고려 같은 건 이 장에서는 없다. (확장 계획은 `MATCHMAKING.md` 참조.)
...
수정은 간단하다. `player_conn.cpp` 의 해당 분기:
...
매치가 성립하면 `relay::startPump(Match)` 가 호출된다. 하는 일:

1. 두 소켓에 `MATCH_FOUND` 전송 (role + seed).
2. 두 개의 `ForwarderLoop` 스레드 spawn — 하나는 `A → B`, 하나는 `B → A`.
...
sequenceDiagram
    ...
    Note over R: startPump(Match{A,B,seed})
    R->>A: MATCH_FOUND(role=HOST, seed)
    R->>B: MATCH_FOUND(role=GUEST, seed)
    Note over A,B: Adopt(sock, role, seed) -> ready
    loop 매 틱 (lockstep)
        A->>R: INPUT(tick N)
        R->>B: INPUT(tick N)
        B->>R: INPUT(tick N)
...
`MATCH_FOUND` 이후는 릴레이가 파싱하지 않는다. 릴레이 큐/룸 경로에서는 seed 가 이미 `MATCH_FOUND` 에 들어 있으므로, 일반 P2P 의 `HELLO/HELLO_ACK/SEED` 핸드셰이크를 다시 하지 않는다.
...
Part 9 에서는 Part 6 의 PyTorch 학습 루프로 훈련한 모델을 ONNX Runtime 으로 들고와, 이 장에서 만든 릴레이에 `QUEUE_JOIN` 으로 접속하는 RL 봇을 완성한다. 서버는 봇인지 사람인지 구분하지 않는다 — Part 8 의 투명 포워딩 덕분에 클라이언트 측 구현만 바꾸면 된다. 봇 입장에서는 Part 5 의 `Session` 과 동일한 프레임 처리 규약 · Part 6 의 정책 네트워크 · Part 8 의 릴레이 접속 세 가지가 만나는 지점이다.
