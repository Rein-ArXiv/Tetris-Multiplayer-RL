...
SIGTERM 회귀 테스트로 고정했다. 남은 가장 큰 유지보수 과제는 `src/main.cpp`의
집중된 책임이다.
...
- first-frame worker도 전역 종료 상태를 확인해 10초 timeout 전에 빠져나온다.
...
| Meta 저장소 | 양호 | DB 직렬화, transaction, migration 및 smoke 테스트 존재 |
...
- C++ 대상 빌드 통과
- 결정론 골든 비교 통과
- Python 전체 스위트: `1661 passed`, `4 skipped`
- skip은 선택 의존성/환경에 따른 테스트이며 실패로 판정하지 않음
...
- 장시간 부하 중 여러 active match가 동시에 종료되는 스트레스 상황
