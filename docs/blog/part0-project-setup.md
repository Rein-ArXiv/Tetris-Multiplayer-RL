...
이 장은 그 모든 재료를 꺼내놓고 빌드 시스템에 줄 세우는 과정을 다룬다. 구체적으로는, 저장소의 `CMakeLists.txt` 302줄을 섹션별로 해부하고, 각 옵션 플래그가 어떤 소스 파일과 어떤 라이브러리를 끌어오는지 추적한다. 독자가 Part 1 로 넘어가기 전에, `cmake -B build && cmake --build build` 가 콘솔에 무엇을 찍는지 전부 설명할 수 있어야 한다.
...
├── CMakeLists.txt         ← 빌드 진입점 (302줄)
├── README.md
├── GUIDE.md               ← 코드 읽는 순서
├── ARCHITECTURE.md        ← 모듈 레퍼런스
├── DEPLOY.md              ← 배포/멀티플레이 실행
├── MATCHMAKING.md         ← 매치메이킹 설계
│
├── core/                  ← 순수 유틸 (외부 의존 없음)
...
저장소의 `CMakeLists.txt` 는 302줄이다. 이 장에서는 이 파일을 섹션별로 발췌하며 전부 설명한다.
...
Part 1 에서는 `platform/win32.cpp` (378줄) 의 전체 내용을 해부한다. `WNDCLASSEXA` 등록부터 `wglCreateContext`, `SwapBuffers`, 메시지 루프, 그리고 엣지-트리거 입력 상태 머신까지. 이 장에서 설정한 `opengl32`, `gdi32`, `winmm`, `gdiplus` 링크가 바로 Part 1 에서 `wglCreateContext`, `ChoosePixelFormat`, `timeBeginPeriod`, `Gdiplus::Bitmap` 의 구체적 사용으로 이어진다.

동시에 SDL2 경로를 쓰는 독자는 Part 1 의 Win32 버전을 참고한 뒤, `platform/sdl.cpp` (303줄) 의 같은 인터페이스 구현을 보면 된다 — 두 파일이 `platform.h` 라는 동일한 함수 시그니처를 공유하므로, 상위 `src/main.cpp` 는 어느 쪽이 링크돼도 바뀌지 않는다. 이 추상화가 이 장에서 세운 `TETRIS_USE_SDL2` 플래그의 실제 효용이다.
