...
- **이번 Part의 파일:** `CMakeLists.txt` 일곱 줄과 스텁 `src/main.cpp`.
- **연결점:** [Part 1](./part1-deterministic-simulation.md) 부터 각 장이 자기 파일을 추가하면서 이 `CMakeLists.txt` 를 조금씩 늘린다. 이 장은 그 출발점만 만든다.
- **완료 게이트:** `cmake -B build && cmake --build build && ./build/tetris` 가 `tetris project skeleton` 한 줄을 출력한다.

이 장에서 게임 코드는 한 줄도 쓰지 않는다. **도구를 갖추고, 그 도구가 실제로 도는지 확인하는 것**이 전부다. 완성된 저장소의 디렉터리 구조나 최종 `CMakeLists.txt` 가 궁금하다면 [Part 13](./part13-structure-and-build-reference.md) 에 레퍼런스로 정리해 뒀다. 다만 그것은 **전부 만든 다음에** 읽는 편이 낫다.
...
이 시리즈는 반대로 간다. 창은 Windows 에서 Win32 API 로 직접 만들고, macOS/Linux 에서는 SDL2 에 **창·입력·최종 화면 복사만** 맡긴다. 도형·텍스트·이미지는 ARGB32 메모리 버퍼에 직접 쓰는 자체 2D 소프트웨어 렌더러가 그린다. 오디오는 XAudio2 또는 SDL audio, 폰트 래스터화는 단일 헤더 `stb_truetype`, 네트워킹은 소켓 API 순정이다.
...
`DrawText("Hello")` 한 줄 뒤에는 폰트 파일 파싱, 글리프 래스터화, 커닝, 베이스라인 계산, 알파 합성, 프레임버퍼 복사가 있다. 엔진을 쓰면 이걸 몰라도 된다 — 잘 돌아갈 때는. 문제는 안 돌아갈 때다. 한 번 직접 써 보면 이후 어떤 엔진을 쓰더라도 오류 메시지가 읽힌다.
...
앞으로 만들 것의 목록이다. 각 항목이 어느 장에서 나오는지도 함께 적었다.
...
| 스프라이트·도형 렌더링 | CPU ARGB32 소프트웨어 래스터라이저 | [Part 3](./part3-rendering-and-ui.md) |
| 폰트 렌더링 | `stb_truetype` + 직접 만든 글리프 캐시 | Part 3 |
...
지금 필요한 것은 **C++ 컴파일러와 CMake** 뿐이다. 나머지는 해당 장에서 필요할 때 안내한다.

### 2.1 Windows

1. [Visual Studio 2022 Community](https://visualstudio.microsoft.com/) 설치. 설치 관리자에서 **"C++를 사용한 데스크톱 개발"** 워크로드를 체크한다. MSVC 컴파일러, Windows SDK, CMake 통합이 전부 여기 들어 있다.
2. Git for Windows.

Windows 에서는 이 두 개면 게임 클라이언트를 끝까지 만들 수 있다. 창(Win32), 화면 출력(GDI), 오디오(XAudio2), 소켓(WinSock2)이 전부 Windows SDK 에 이미 들어 있어서 **추가로 설치할 라이브러리가 없다.** 이것이 Win32 백엔드를 유지하는 이유 중 하나다 — 배포할 때 동봉할 DLL 이 없다.
...
Visual Studio 를 설치했는데 `cmake` 를 못 찾는다면 일반 PowerShell 대신 **x64 Native Tools Command Prompt for VS 2022** 를 열면 된다.
...
OpenGL 개발 패키지는 **필요 없다.** 렌더링을 전부 CPU 에서 하기 때문이다. `libgl1-mesa-dev` 같은 것을 깔라는 안내를 보더라도 이 프로젝트에는 해당하지 않는다.
...
Xcode 전체를 설치할 필요는 없다. command line tools 면 충분하다.

### 2.4 Python — 나중에 필요하지만 지금 깔아두면 편하다
...
이 프로젝트는 그 경계를 **최상위 디렉터리로** 강제한다. 규칙 코드가 들어갈 디렉터리는 렌더링 코드가 들어갈 디렉터리를 include 하지 않는다. 나중에 빌드 파일이 이 경계를 한 번 더 못박는다 — 규칙만 모아 빌드하는 타깃이 따로 있어서, 경계를 어기면 **링크 에러로 즉시 드러난다.**

지금 디렉터리를 미리 만들 필요는 없다. 각 장에서 필요할 때 하나씩 생긴다. 어떤 것들이 생기는지는 §1 의 표에 이미 나와 있고, 다 만든 뒤의 전체 지도는 [Part 13](./part13-structure-and-build-reference.md) 에 있다.

## 4. 첫 체크포인트 — 툴체인이 도는지 확인한다

여기서 확인할 것은 하나다. **컴파일러와 CMake 가 실제로 동작하는가.**

최종 `CMakeLists.txt` 를 지금 복사해 오면 안 된다. 아직 만들지 않은 수십 개의 소스 파일을 찾다가 configure 단계에서 죽는다. 실행 파일 하나짜리로 시작한다.
...
이 일곱 줄이 앞으로 계속 자란다. **파일을 새로 만드는 장은 그 장 안에서 빌드 파일도 함께 고친다.** [Part 1](./part1-deterministic-simulation.md) 이 게임 규칙 소스를 더하고, [Part 2](./part2-platform-window-input.md) 가 창을 띄우는 파일을 더하는 식이다.
...
- 일곱 줄짜리 `CMakeLists.txt` 와 스텁 `src/main.cpp` 로 configure → build → 실행이 성공한다.
- 앞으로 무엇을 직접 만들게 되는지, 그 이유가 무엇인지 알고 있다.
